...
def main():
    ...
    class HomonymTrainer(Trainer):
        def compute_loss(self, model, inputs, return_outputs=False):
            ...
            loss_fct = nn.CrossEntropyLoss(weight=torch.tensor([1.0, 0.8, 1.0]))
            loss = loss_fct(logits.view(-1, self.model.config.num_labels), labels.view(-1))
            return (loss, outputs) if return_outputs else loss
    
    trainer_class = HomonymTrainer
    ...
    if training_args.do_train:
        ...
        with open(os.path.join(training_args.output_dir, 'train_data.jsonl'), encoding='utf-8', mode='w') as f:
